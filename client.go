...
import (
	...
	"github.com/netapp-polaris/gocloak/v13/pkg/jwx"
)
...
func (g *GoCloak) CreateUserFederation(ctx context.Context, accessToken, realm string, request CreateUserFederationRequest) (string, error) {
...
func (g *GoCloak) GetUserFederation(ctx context.Context, accessToken, realm string) (*Component, error) {
	var componentDetails *Component
	components, err := g.GetComponents(ctx, accessToken, realm)
	if err != nil {
	...
	for _, component := range components {
		...
		if "ldap" == providerId {
			...
			realmDetails, err := g.GetRealm(ctx, accessToken, realm)
			if err != nil {
				return componentDetails, err
			}
...
func (g *GoCloak) CheckLdapConnection(ctx context.Context, accessToken, realm string, reqBody CheckLdapConnection) error {
	const errMessage = "could not test ldap connection"

	resp, err := g.GetRequestWithBearerAuth(ctx, accessToken).
		SetBody(reqBody).
		Post(g.getAdminRealmURL(realm, "testLDAPConnection"))
	if err := checkForError(resp, err, errMessage); err != nil {
		return err
	}
	return nil
...
func (g *GoCloak) SyncLdapUsers(ctx context.Context, accessToken, realm string, providerId *string, queryParam SyncLdapUsersParams) (*SyncLdapUsersGroupsResponse, error) {
	...
	resp, err := g.GetRequestWithBearerAuth(ctx, accessToken).
		...
		SetResult(&result).
		Post(g.getAdminRealmURL(realm, "user-storage", *providerId, "sync"))
...
func (g *GoCloak) AddGroupMapper(ctx context.Context, accessToken, realm string, reqBody LdapGroupMapper) error {
...
func (g *GoCloak) GetLdapMappers(ctx context.Context, accessToken, realm string, queryParams GetLdapMapperParams) ([]*LdapMapperDetail, error) {
...
func (g *GoCloak) SyncLdapGroups(ctx context.Context, accessToken, realm, mapperId string, parentId *string, queryParam SyncLdapGroupParam) (*SyncLdapUsersGroupsResponse, error) {
...
func (g *GoCloak) ModifyLdapDetails(ctx context.Context, accessToken, realm string, ldapId *string, reqBody Component) error {
	const errMessage = "could not modify ldap user federation details"

	resp, err := g.GetRequestWithBearerAuth(ctx, accessToken).
		SetBody(reqBody).
		Put(g.getAdminRealmURL(realm, "components", *ldapId))
...
func (g *GoCloak) ModifyMapperDetails(ctx context.Context, accessToken, realm, mapperId string, reqBody LdapMapperDetail) error {
	const errMessage = "could not modify ldap group mapper details"

	resp, err := g.GetRequestWithBearerAuth(ctx, accessToken).
		SetBody(reqBody).
		Put(g.getAdminRealmURL(realm, "components", mapperId))
